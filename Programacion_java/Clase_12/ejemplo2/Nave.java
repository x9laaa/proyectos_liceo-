...
public class Nave {
    ...
    public void asignarImpostor() {
        if (!tripulantes.isEmpty()) {
            int indiceImpostor = (int) (Math.random() * tripulantes.size());
            tripulantes.get(indiceImpostor).setInspoctor(true);
            System.out.println("Inpostor asignado......;)");
        } else {
            System.out.println("No hay tripulantes para asignar como impostor.");
        }
    }

    public void cantidadImpostores() {
        int contador = 0;
        for (Tripulante tripulante : tripulantes) {
            if (tripulante.isInspoctor()) {
                contador++;
        ...
        System.out.println("Cantidad de impostores en la nave " + nombre + ": " + contador);
    }

    public boolean verificarTripulante(String nombre) {
        for (Tripulante tripulante : tripulantes) {
            if (tripulante.getNombre().equals(nombre)) {
                if (tripulante.isInspoctor()) {
                    return true;
                }
